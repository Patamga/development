...
const Dummy = () => {
  ...
  01 : 59
  01 : 58
  и так далее. В любой момент можно нажать на кнопку паузу -
...
функцию https://developer.mozilla.org/en-US/docs/Web/
...
использование setInterval в подобных задачах - считается ошибкой, тк setInterval не гарантирует выховы в равные интервалы времены и чем дольше работает setInterval, тем больше смещение будет.`
  ...
  return (
    <div className=" w-ful p-5 flex flex-col wrap text-xs">
      {text5}
      {part2}
    </div>
  )
