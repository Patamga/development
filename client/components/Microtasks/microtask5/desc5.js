...
const Dummy = () => {
  ...
  Одно поле в котором задаются минуты. И одна кнопка Старт. Как только пользователь нажимает старт, появляются два дива один с двумя дивами с большими цифрами - минуты и секунды другой с кнопкой паузы
  Каждую секунду идет отсчет назад
  01 : 59
  01 : 58
  и так далее. В любой момент можно нажать на кнопку паузу - таймер остановится и снова на нее нажать таймер начнется
  Как только таймер доходит до 00 : 00 - таймер останавливается - выводится сообщение FINISHED под часами

Для шрифтов используйте шрифт

Roboto - typeface-roboto
Для форматирования 0 впереди используйте функцию https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/padStart
`
const part2 = `
План действий
...
чтиво по теме - https://learn.javascript.ru/settimeout-setinterval
...
использование setInterval в подобных задачах - считается ошибкой, тк setInterval не гарантирует выховы в равные интервалы времены и чем дольше работает setInterval, тем больше смещение будет.`
  ...
  return <div className="p-2 flex">{text5}{part2}</div>
}
